...
///
/// Returns `Ok(None)` for events we don't model — e.g. a non-standard
/// `ping` keep-alive heartbeat that a third-party gateway (such as
/// OpenCode Zen) may emit. Serde rejects any `type` not in the
/// `rs::ResponseStreamEvent` enum, so we skip it rather than abort the whole
/// stream. This mirrors OpenCode's "ignore unknown event types" behaviour.
pub(crate) fn deserialize_response_event(data: &str) -> Result<Option<rs::ResponseStreamEvent>> {
    let mut event = match serde_json::from_str::<rs::ResponseStreamEvent>(data) {
        Ok(event) => event,
        Err(first_err) => {
            // OpenCode Zen emits a `{"type":"ping","cost":"0"}` keep-alive
            // heartbeat that serde rejects (the `type` isn't in the
            // `rs::ResponseStreamEvent` enum). Match the exact payload shape
            // rather than the serde error substring — the substring also
            // matches nested enum mismatches in tool/usage fields, which are
            // real parse failures and must not be silently dropped.
            if let Ok(value) = serde_json::from_str::<serde_json::Value>(data)
                && value.get("type").and_then(|v| v.as_str()) == Some("ping")
            {
                return Ok(None);
            }
                    ...
                    Ok(event) => {
                        ...
                        if swallow {
                            ...
                            match deserialize_response_event(data) {
                                ...
                                // Unrecognized event (e.g. a `ping`
                                // heartbeat) — drop it without ending the stream.
                                Ok(None) => Some(None),
...
mod tests {
    ...
    #[test]
    fn deserialize_response_event_skips_unknown_event_types_like_ping() {
        // Third-party gateways (e.g. OpenCode Zen) emit non-standard
        // keep-alives. These aren't in the `rs::ResponseStreamEvent` enum,
        // so they used to abort the whole stream with a "unknown variant"
        // serialization error. They must be skipped (Ok(None)) instead.
        // Exact heartbeat observed on the wire: SSE `event: ping` with this
        // data payload, sent after `response.completed`.
        let ping = r#"{"type":"ping","cost":"0"}"#;
        assert!(matches!(deserialize_response_event(ping), Ok(None)));

        // A malformed *known* event is still an error, not silently skipped.
        let broken = r#"{"type": "response.completed"}"#;
        assert!(deserialize_response_event(broken).is_err());
    }
}
