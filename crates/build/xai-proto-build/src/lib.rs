...
impl XaiProtoBuilder {
    ...
    //   rather than the actual proto file location.
    fn emit_rerun_if_changed<'a>(
        ...
        #[cfg(windows)]
        {
            // `/dev/stdout` / `/dev/null` are not available on Windows, so skip
            // protoc --dependency_out and rerun on the listed proto/include paths.
            for proto in protos {
                println!(
                    "cargo:rerun-if-changed={}",
            ...
            for include in &includes {
                println!(
                    "cargo:rerun-if-changed={}",
                    include.to_str().context("include path not UTF-8")?
                );
            }
            return Ok(());
        }
